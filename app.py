...
def yami_pain_bucket_exercicio(ex: str, pain_flags: dict | None) -> int:
    """Bucket de dor por exercício (0/1/2), com base nos sinais do corpo relevantes."""
    try:
        ...
        relevantes = set()
        if fam in ("squat", "hinge", "glute"):
            relevantes.update(["joelho", "lombar"])
        if fam in ("push_h", "push_v"):
            relevantes.update(["ombro", "cotovelo"])
        if fam in ("pull_h", "pull_v"):
            relevantes.update(["cotovelo", "ombro"])
        if any(k in exl for k in ["curl", "bíceps", "biceps", "tríceps", "triceps", "pushdown", "extensão"]):
            relevantes.add("cotovelo")

        n = sum(1 for a in relevantes if bool(pain_flags.get(a, False)))
        if bool(pain_flags.get("lombar", False)) and ("lombar" in relevantes):
            return 2
        if n >= 2:
            return 2
        if n == 1:
...
def yami_contexto_decisao(
    ...
    """Contexto único para o Yami decidir (semana + prontidão + sinais do corpo + histórico).

    Evita conflitos onde uma parte do Yami considera X e outra ignora.
    """
    read = st.session_state.get("yami_readiness", {}) or {}
    chk_adj = float(read.get("adj_load_pct", 0.0) or 0.0)
    chk_score = float(read.get("score_delta", 0.0) or 0.0)
    chk_lbl = str(read.get("label", "Normal") or "Normal")
    chk_rir = float(read.get("adj_rir", 0.0) or 0.0)
    ...
    lat_lbl = str(lat.get("label", "latente: normal") or "latente: normal")
    z = float(lat.get("z", 0.0) or 0.0)
    lat_rir = 0.5 if z <= -1.0 else 0.25 if z <= -0.5 else (-0.25 if z >= 1.0 else (-0.10 if z >= 0.5 else 0.0))

    pain_flags = st.session_state.get("yami_pain_flags", {}) or {}
    pb = int(yami_pain_bucket_exercicio(ex, pain_flags))
    pain_adj = (-0.02 if pb == 1 else -0.04 if pb >= 2 else 0.0)
    pain_score = (-0.20 if pb == 1 else -0.35 if pb >= 2 else 0.0)
    pain_rir = (0.5 if pb == 1 else 1.0 if pb >= 2 else 0.0)
    sigma_mult = (1.15 if pb == 1 else 1.35 if pb >= 2 else 1.0)
    ...
    try:
        ...
        week_pen = 0.0
    week_adj = -0.01 * min(1.0, week_pen) if week_pen > 0 else 0.0

    strength_adj = max(-0.08, min(0.04, 0.8 * chk_adj + 1.0 * lat_adj + 0.9 * pain_adj))
    load_adj = max(-0.12, min(0.06, chk_adj + lat_adj + pain_adj + week_adj))
    score_delta = float(chk_score + lat_score + pain_score)
    adj_rir = float(chk_rir + lat_rir + pain_rir)

    bits = [chk_lbl] if chk_lbl else []
    if lat_lbl and "normal" not in lat_lbl:
        bits.append(lat_lbl)
    if pb > 0:
        bits.append("sinais do corpo")
    if week_pen >= 0.25:
        bits.append("semana carregada")
    label = " · ".join(bits) if bits else "Normal"
    ...
    return {
        "label": str(label),
        "adj_load_pct": float(load_adj),
        "strength_adj_pct": float(strength_adj),
        "score_delta": float(score_delta),
        "adj_rir": float(adj_rir),
        "pain_bucket": int(pb),
        "pain_risk": float(pain_risk),
        "sigma_mult": float(sigma_mult),
        "week_pen": float(week_pen),
    }


def yami_contexto_cached(df_hist: pd.DataFrame | None, perfil: str, ex: str, item: dict, bloco: str = "", semana: int = 1) -> dict:
    """Cache leve do contexto para não recalcular em loop de séries."""
    try:
        read = st.session_state.get("yami_readiness", {}) or {}
        pain = st.session_state.get("yami_pain_flags", {}) or {}
        key = (
            str(perfil),
            str(ex),
            str(bloco),
            ...
            str(item.get("tipo","")),
            str(item.get("reps","")),
            float(read.get("adj_load_pct",0.0) or 0.0),
            float(read.get("score_delta",0.0) or 0.0),
            str(read.get("label","")),
            int(bool(pain.get("joelho",False))),
            int(bool(pain.get("cotovelo",False))),
            int(bool(pain.get("ombro",False))),
            int(bool(pain.get("lombar",False))),
        )
        cache = st.session_state.get("_yami_ctx_cache", {}) or {}
        if key in cache:
            return cache[key]
        ctx = yami_contexto_decisao(df_hist, perfil, ex, item, bloco=bloco, semana=semana)
        cache[key] = ctx
        if len(cache) > 64:
            cache = dict(list(cache.items())[-32:])
        st.session_state["_yami_ctx_cache"] = cache
        return ctx
    except Exception:
        return yami_contexto_decisao(df_hist, perfil, ex, item, bloco=bloco, semana=semana)
...
def yami_twin_predict(
    ...
    sig0 = float(pred.get("sigma", 999.0) or 999.0)
    ...
    if isinstance(ctx, dict):
        strength_adj = float(ctx.get("strength_adj_pct", 0.0) or 0.0)
        sigma_mult = float(ctx.get("sigma_mult", 1.0) or 1.0)
        ctx_label = str(ctx.get("label", "") or "")

    e_today = mu * (1.0 + r_lat + strength_adj)
    ...
    p_phys = _sigmoid(2.6 * margin)

    sig = float(sig0) * float(sigma_mult)

    return {
        "p_phys": float(p_phys),
        "rir_pred": float(rir_pred),
        "reps_to_fail": float(reps_fail),
        "mu": float(mu),
        "sigma": float(sig),
        "readiness": float(r_lat + strength_adj),
        "daytype": dt,
        "ctx_label": ctx_label,
    }
...
def yami_pick_weight_optimized(
    ...
    if isinstance(ctx, dict):
        ...
        try:
            ...
            pass
    pred = yami_kalman_predict(perfil, ex)
    twin = yami_twin_predict(perfil, ex, base_weight, target_reps, rir_target, rest_s, set_idx, total_sets, ctx=ctx)
    pref = yami_pref_get(perfil)
    ...
    for w in cands:
        twin_w = yami_twin_predict(perfil, ex, w, target_reps, rir_target, rest_s, set_idx, total_sets, ctx=ctx)
...
def yami_week_stats(df_hist: pd.DataFrame, perfil: str) -> dict:
    ...
    try:
        ...
        for _, r in dfw.iterrows():
            ...
            w_list = _yami_cell_to_list(r.get("Peso"))
            r_list = _yami_cell_to_list(r.get("Reps"))
            load = 0.0
            if w_list and r_list:
                for wi, ri in zip(w_list, r_list):
                    try:
                        load += max(0.0, float(wi)) * max(0.0, float(ri))
                    except Exception:
                        pass
            else:
                w = _yami_parse_float_pt(r.get("Peso")) or 0.0
                reps = _yami_parse_float_pt(r.get("Reps")) or 0.0
                load = max(0.0, float(w) * float(reps))
            load_total += load
...
def _yami_suggest_weight_for_series(
    ...
    ctx = yami_contexto_cached(df_hist, perfil, ex, item, bloco=str(bloco or ''), semana=int(semana))

    pain_bucket = int((ctx.get('pain_bucket', 0) if isinstance(ctx, dict) else 0) or 0)
    week_pen = float((ctx.get('week_pen', 0.0) if isinstance(ctx, dict) else 0.0) or 0.0)

    try:
        adj = float(ctx.get('adj_load_pct', 0.0) or 0.0) if isinstance(ctx, dict) else 0.0
    except Exception:
        adj = 0.0
    if abs(adj) > 1e-9 and base > 0:
        base = float(_yami_apply_load_adj_step(float(base), float(_yami_granularidade_peso(ex, item)), float(adj)))
    ...
    try:
        opt = yami_pick_weight_optimized(
            ...
            bloco=str(bloco or ''),
            pain_risk=float((ctx.get('pain_risk', 0.0) if isinstance(ctx, dict) else 0.0) or 0.0),
...
def _yami_apply_load_adj_step(weight: float, gran: float, adj_pct: float) -> float:
    """Aplica um ajuste percentual e garante efeito visível quando o arredondamento anula o ajuste.
    Útil para prontidão/sinais do corpo em granularidades grandes (ex.: 5kg).
    """
    try:
        w = float(weight)
        g = max(0.5, float(gran))
        a = float(adj_pct)
        w2 = float(_round_to_nearest(w * (1.0 + a), g))
        # se o ajuste é relevante mas o arredondamento anulou, força 1 passo
        if abs(a) >= 0.015 and abs(w2 - w) < 1e-9:
            if a < 0:
                w2 = max(0.0, w - g)
            elif a > 0:
                w2 = max(0.0, w + g)
...
def _yami_normaliza_num_pt(s: str) -> str:
    """Normaliza números em PT-PT (vírgula decimal) e lixo comum (kg, espaços, etc.)."""
    s = str(s or "").strip()
    if not s:
        return ""
    s = s.replace("−", "-")
    ...
    s = re.sub(r"\bkg\b", "", s, flags=re.IGNORECASE)
    # vírgula decimal -> ponto decimal (apenas entre dígitos)
    s = re.sub(r"(?<=\d),(?=\d)", ".", s)
    return s.strip()

def _yami_parse_float_pt(v):
    """Converte um valor para float respeitando vírgula decimal PT-PT. Retorna None se não der."""
    try:
        if v is None or (isinstance(v, float) and pd.isna(v)):
            return None
        if isinstance(v, (int, float)) and not (isinstance(v, float) and pd.isna(v)):
            return float(v)
        s = _yami_normaliza_num_pt(str(v))
        if not s:
            return None
        m = re.search(r"(-?\d+(?:\.\d+)?)", s)
...
def _yami_cell_to_list(v) -> list:
    ...
    - Suporta decimal PT (ex.: 72,5)
    - Suporta listas em célula (legado): "70;72,5" | "70/72,5" | "70 72,5"
    """
    try:
        if v is None or (isinstance(v, float) and pd.isna(v)):
            return []
        if isinstance(v, (int, float)) and not (isinstance(v, float) and pd.isna(v)):
            return [float(v)]
        s = _yami_normaliza_num_pt(str(v).strip())
        if not s:
            return []
        # Separadores de lista (depois de normalizar decimais)
        s = s.replace("|", ";").replace("/", ";")
        # vírgulas restantes agora são separadores (não decimais)
        s = s.replace(",", ";")
        # Se vier como "70 72.5" (espaço), tratar como lista
        if re.search(r"\b\d+(?:\.\d+)?\s+\d+(?:\.\d+)?\b", s):
            s = re.sub(r"\s+", ";", s)
        parts = [p.strip() for p in s.split(";") if p.strip()]
        out = []
        for p in parts:
            fv = _yami_parse_float_pt(p)
            if fv is not None:
                out.append(float(fv))
        return out
    except Exception:
        ...
        if dfp.empty:
            ...
            for c in ["Peso", "Reps", "RIR"]:
                if c in dfp.columns:
                    ...
                    conv = dfp.loc[~mask_list, c].apply(_yami_parse_float_pt)
                    dfp.loc[~mask_list, c] = pd.to_numeric(conv, errors="coerce")
...
def _historico_resumos_exercicio(df: pd.DataFrame, perfil: str, ex: str) -> list:
    ...
    d0['__ex_norm'] = d0['Exercício'].astype(str).str.strip()
    d = d0[d0['__ex_norm'] == str(ex).strip()].copy()
    if d.empty:
        return []
...
def yami_coach_sugestao(df_hist: pd.DataFrame, perfil: str, ex: str, item: dict, bloco: str, semana: int, plano_id: str) -> dict:
    ...
    # contexto unificado (prontidão + sinais do corpo + semana + histórico)
    ctx = yami_contexto_cached(df_hist, perfil, ex, item, bloco=str(bloco or ''), semana=int(semana))
    ...
    try:
        read_adj_pct = float(ctx.get("adj_load_pct", 0.0) or 0.0)
    except Exception:
        read_adj_pct = 0.0
    try:
        read_score_delta = float(ctx.get("score_delta", 0.0) or 0.0)
    except Exception:
        read_score_delta = 0.0
    try:
        read_label = str(ctx.get("label", read_label) or read_label)
    except Exception:
        pass

    # ajusta RIR alvo com o contexto (dia mau/dor -> mais conservador)
    try:
        rir_alvo_num_ = float(rir_alvo_num_) + float(ctx.get("adj_rir", 0.0) or 0.0)
        rir_alvo_num_ = max(0.0, min(6.0, float(round(rir_alvo_num_ * 2) / 2.0)))
    except Exception:
        pass
    ...
    if not latest or (float(latest.get('peso_medio', 0) or 0) <= 0 and len(list(latest.get('reps', []) or [])) == 0):
        return {
    ...
    try:
        if deload_force:
            ...
            if abs(float(read_adj_pct)) > 1e-9:
                _pre = float(p_sug)
                _adj = float(_yami_apply_load_adj_step(float(p_sug), float(gran), float(read_adj_pct)))
