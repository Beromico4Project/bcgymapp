...
st.markdown("""
    ...
    * { font-family: 'MedievalSharp', cursive !important; }
    h1, h2, h3, .stHeadingContainer { 
        font-family: 'Cinzel', serif !important; color: #FF4B4B !important; 
        text-shadow: 2px 2px 0px #000; text-transform: uppercase; font-weight: 900 !important;
    }
    p, label, span, div { color: #E0E0E0 !important; }
    
    /* Abas */
    .stTabs [data-baseweb="tab-list"] { gap: 8px; background-color: rgba(0,0,0,0.5); padding: 10px; border-radius: 10px; border: 1px solid #444; }
    .stTabs [data-baseweb="tab"] { height: 50px; background-color: rgba(60,60,60,0.8); border: 1px solid #555; color: #ddd; }
    .stTabs [aria-selected="true"] { background-color: rgba(139,0,0,0.9) !important; color: #FFD700 !important; border: 2px solid #FF0000 !important; }
    
    /* Inputs */
    .stTextInput input, .stNumberInput input, .stTextArea textarea { background-color: rgba(0,0,0,0.6) !important; color: white !important; border: 1px solid #666 !important; }
    
    /* Botões */
    div.stButton > button:first-child { background: linear-gradient(180deg, #8B0000 0%, #400000 100%) !important; color: #FFD700 !important; border: 1px solid #FF4B4B !important; font-family: 'Cinzel', serif !important; }
    header { background: transparent !important; }
    </style>
""", unsafe_allow_html=True)

# --- 4. DADOS E LÓGICA NOVA ---
...
def get_treino_anterior(exercicio):
    ...
    # (Assumindo que o registo mais recente está no fim do CSV)
    ...
    # Retorna o DataFrame filtrado e o peso da última série para sugestão
    return treino_passado[["Peso", "Reps", "RPE", "Notas"]], float(ultima_linha["Peso"])
...
with tab_treino:
    if dia == "Descanso":
        ...
        for i, item in enumerate(treino_hoje):
            ...
            # --- MUDANÇA: BUSCAR TODO O HISTÓRICO ANTERIOR ---
            ...
            with st.expander(f"{i+1}. {nome_display}", expanded=(i==0)):
                ...
                # --- MOSTRAR TABELA DO TREINO ANTERIOR ---
                if df_passado is not None:
                    st.markdown("📜 **Grimório Passado:**")
                    # Mostra a tabela limpa sem índice numérico feio
                    st.dataframe(df_passado, hide_index=True, use_container_width=True)
                else:
                    st.caption("Primeira vez neste feitiço.")

                with st.form(key=f"form_{i}"):
                    cc1, cc2, cc3 = st.columns([1,1,2])
                    # Usa o peso sugerido da última vez como padrão
